...
# This script adding a third organization to the network previously 
...
function printHelp () {
  echo "Usage: "
  echo "  add_Org3.sh up|down|restart|generate [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>]"
  echo "  add_Org3.sh -h|--help (print this message)"
  ...
  echo
  echo "	add_Org3.sh generate -c mychannel"
  echo "	add_Org3.sh up -c mychannel -s couchdb"
  echo "	add_Org3.sh up -l node"
  echo "	add_Org3.sh down -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	add_Org3.sh generate"
  echo "	add_Org3.sh up"
  echo "	add_Org3.sh down"
}
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images|awk '($1 ~ /dev-peer.*.assetscc.*/) {print $3}')
...
# be parked in the folder titled ``crypto-config``.
...
if [ ! -d crypto-config ]; then
  echo
  echo "ERROR: Please, run startFabric.sh first."
  echo
  exit 1
fi
